      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:gap-6 sm:px-8">
        ...
        <div className="relative ml-auto">
          <button
            ...
            className="flex items-center gap-2 rounded-full border border-slate-200 bg-white py-1 pl-1.5 pr-2 text-sm hover:border-brand-200 sm:gap-3 sm:pr-3"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-600 text-xs font-semibold text-white">
              {initials}
            </span>
            <span className="hidden text-left sm:block max-w-[140px] lg:max-w-[220px]">
      ...
      {mobileNavOpen && (
        <>
          <div
            ...
            className="fixed inset-0 top-16 z-20 bg-slate-900/20 backdrop-blur-sm md:hidden"
            aria-hidden="true"
          />
          <div className="absolute left-0 right-0 top-full z-30 border-b border-slate-200 bg-white shadow-lg md:hidden">
            <nav className="mx-auto flex max-w-7xl flex-col gap-1 px-4 py-3 sm:px-8">
              {navItems.map((item) => (
                <NavLink
                  key={item.to}
                  to={item.to}
