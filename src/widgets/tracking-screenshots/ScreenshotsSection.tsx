...
import { trackingSettingsApi } from '@/entities/tracking-settings'
...
function findActivity(
  ...
  let best: ActivityLog | undefined
...
export default function ScreenshotsSection() {
  ...
  const settingsQuery = useQuery({
    ...
    queryFn: trackingSettingsApi.get,
    staleTime: 5 * 60_000,
  })
  ...
  const { data, isLoading, isFetching, error } = useQuery({
    ...
    queryFn: () => activityApi.listScreenshots(screenshotsParams),
  })

  const activityQuery = useQuery({
    queryKey: ['screenshots-activity', applied],
    queryFn: () => activityApi.listActivity(screenshotsParams),
  })
